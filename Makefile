.PHONY: install dev demo demo-anthropic demo-code demo-agy test docker deploy-check reset providers railway-check web-install web-dev web-build web-test
...
# Los perfiles. El proveedor es un flag, no una pantalla de ajustes.
demo:            ; PYTHONPATH=src uvicorn harness.app:app --port 8080
...
railway-check:   ; curl -fsS $${RAILWAY_URL:-localhost:8080}/readyz | python -m json.tool
...
web-test:        ; cd frontend && npm run test
